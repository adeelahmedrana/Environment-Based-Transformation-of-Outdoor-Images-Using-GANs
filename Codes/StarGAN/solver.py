        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(data_loader):
                ...
                for j, c_trg in enumerate(c_trg_list):
                    x_fake_list = self.G(x_real, c_trg)
                    for k in range(len(x_fake_list)):
                        sample_path = os.path.join(self.result_dir, f'{i+1}-{k}-feat{j}-image.jpg')
                        save_image(self.denorm(x_fake_list.data[k].cpu()), sample_path, nrow=1, padding=0)
